...
Razor Designer is a s&box editor library that adds a dockable visual designer for `.razor` UI panels. You build a panel by adding controls to a canvas, arranging them in a hierarchy, and editing their properties in the inspector. 

The designer writes a paired `.razor` and `.razor.scss` into your project.
...
- **Real preview, not a mockup.** The canvas renders with actual
  ...
  runtime. What you see in the designer is what you ship. *Yes, this includes animations and transition states.*
...
- **Reusable templates.** Any combination of controls can be saved as a palette template
  and reused across designs.
